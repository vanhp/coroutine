...
class PlantListViewModel internal constructor(
    ...
    private val growZoneChannel = ConflatedBroadcastChannel<GrowZone>() // keep only the latest value
    val plantsUsingFlow: LiveData<List<Plant>> = growZoneChannel.asFlow()
            .flatMapLatest { growZone -> // work like switchmap on livedata but for flow it allows
                if(growZone == NoGrowZone){ //switch to diff flow base on value
                    plantRepository.plantsFlow
                }else{
                    plantRepository.getPlantsWithGrowZoneFlow(growZone)
                }
            }.asLiveData() // convert flow to livedata with configurable timeout to keep the flow
                            // active through switching so collection doesn't restart
    ...
    init {
        ...
        growZoneChannel.asFlow()
                // maplatest may be used to control currency instead of create one by hand
                .mapLatest { growZone ->  // launch new coroutine to apply map to each item
                                            // or cancel current one if new value is emit
                                            // before completion then start a new one
                ...
                .onCompletion { _spinner.value = false } //clean up
                .catch { throwable -> _snackbar.value = throwable.message } // exception
                .launchIn(viewModelScope) //terminal op to start and collect value
    }
...
//    }
    ...
    // using flow version
    // to communicate change of event to coroutine via offer
    fun setGrowZoneNumber(num: Int){
        growZone.value = GrowZone(num)
        growZoneChannel.offer(GrowZone(num))
        // this's for livedata using flow now don't need it
