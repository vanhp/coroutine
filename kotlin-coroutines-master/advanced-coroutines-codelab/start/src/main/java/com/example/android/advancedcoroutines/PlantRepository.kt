    ...
    val plantsFlow: Flow<List<Plant>>
       get() = plantDao.getPlantsFlow()
           ...
           .combine(customSortFlow) { plants, sortOrder ->
                    plants.applySort(sortOrder) }
               .flowOn(defaultDispatcher) // has buffer may create faster than consumer
               .conflate() //modify buffer to store only latest item
...
//        val customSortOrder = plantsListSortOrderCache.getOrAwait()
//        emitSource(plantsGrowZoneLiveData.map { plantList -> plantList.applySort(customSortOrder) })
//    }
    fun getPlantsWithGrowZone(growZone: GrowZone) =
            plantDao.getPlantsWithGrowZoneNumber(growZone.number)
                    .switchMap { plantList ->
    ...
    // custom sorted order by placing the prefer items first before others items
    private fun List<Plant>.applySort(customSortOrder: List<String>): List<Plant> {
        return sortedBy { plant ->
            val positionForItem = customSortOrder.indexOf(plant.plantId).let{
                                    order -> if (order > -1) order else Int.MAX_VALUE }
